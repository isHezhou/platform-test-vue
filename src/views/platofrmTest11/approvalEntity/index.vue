  ...
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-form-item label="编码">
          <el-input v-model="listQuery.csCode" placeholder="编码"
                    class="filter-item"
...
<div class="table-container">
    ...
    <pagination
       ...
       @pagination="doQueryList"/>
        <!-- 查看表单 -->
        <approvalEntity-show ref="approvalEntityShow"/>
        <!-- 新建表单 -->
        <approvalEntity-add ref="approvalEntityAdd"
                                @created="doQueryList({ page: 1 })"/>
        <!-- 编辑表单 -->
        <approvalEntity-edit ref="approvalEntityEdit"
                                @updated="doQueryList({})"/>
    <!-- 详情表单 -->
    <Tabs ref="tabs" :params="params" @doQueryList="doQueryList({})"/>
    <!-- 撤销表单 -->
    <approvalEntity-undo ref="approvalEntityUndo" @undo="doQueryList({})"/>
  </div>
</template>

<script>
import approvalEntityAdd from './add'
import approvalEntityEdit from './edit'
import approvalEntityShow from './show'
import Tabs from '@/components/Tabs/index'
import approvalEntityUndo from './undo'
...
export default {
  ...
  data() {
    return {
      params: {
        'businessKey': '87ceea2b0c114fc89eebb12f88958122',
  ...
  methods: {
    ...
    doQueryList({ page, limit }) {
      ...
      this.listLoading = true
      return approvalEntityApi.finds(args)
        .then(res => {
    ...
    handleDeleteBatch() {
      if (this.selectItems.length <= 0) {
        this.$common.showMsg('warning', '请选择普通审批菜单')
        return
      }
      return this.$common.confirm('是否确认删除')
        .then(() =>{
            this.listLoading = true
            approvalEntityApi.deleteBatch(this.selectItems.map(row => row.id))
            .then(() => {
                this.$common.showMsg('success', '删除成功')
                return this.doQueryList({ page: 1 })
            }).catch(()=>{
     ...
     */
    handleCreate() {
      this.$refs.approvalEntityAdd.handleCreate()
    },
    /**
     * 打开查看表单
     */
    handleShow(row) {
      this.$refs.approvalEntityShow.handleShow(row)
    },
    /**
     * 打开编辑表单
     */
    handleUpdate(row) {
      this.$refs.approvalEntityEdit.handleUpdate(row)
    },
    /**
     * 根据数据状态按钮是否隐藏
     */
    ...
    handleSubmit(row) {
      this.$common.confirm('是否确认提交')
        .then(()=>{
          ...
          this.remoteCallPayload.url = 'http://' + defaultSettings.gateway + '/platofrmTest11/approvalEntity/updateByWorkflow'
          this.remoteCallPayload.token = 'Bearer ' + getAccessToken()

          form.remoteCallPayload = this.remoteCallPayload
          delete form.hiddenAttrs
          //调用提交接口
          approvalEntityApi.submit(form).then(res=>{
            //未找到流程实例
            if('000000' === res.code){
                this.doQueryList({ page: 1 });
                this.$message({
    ...
    handleDetails(row) {
      ...
      const params = {
        'businessKey': '87ceea2b0c114fc89eebb12f88958122' + row.id,
        'variable-name': 'workflowPrefix_form'
      }
     ...
     */
    handleUndo(row) {
      this.$refs.approvalEntityUndo.handleCreate(this.params.businessKey + row.id)
    },
    /**
     * 结束业务
     *
    ...
    handEndProcess(row) {
      this.$common.confirm('未找到流程实例,是否结束业务')
        .then(()=>{
          this.listLoading = true
          approvalEntityApi.endProcess(row).then(res=>{
            if(res.code === '000000'){
              this.doQueryList({ page: 1 });
              this.$message({
                type: 'success',
    ...
    handleRushDo(row) {
      this.$common.confirm('是否催办')
        .then(()=>{
          this.listLoading = true
          approvalEntityApi.rushDo(this.params.businessKey + row.id).then(res=>{
            if(res.code === '000000'){
              this.$message({
                type: 'success',
                message: '催办成功!'
    ...
    exportAllExcel() {
      ...
      args.q = queryToString(this.listQuery)
      approvalEntityApi.exportExcelAll(args).then(res=>{
        this.listLoading = false
        let link = document.createElement('a');
        let body = document.querySelector('body');
        link.href = window.URL.createObjectURL(res);
        link.download = "普通审批菜单记录.xlsx";
        // fix Firefox
    ...
    export(pageNum) {
      ...
      approvalEntityApi.exportExcel(args).then(res=>{
        this.listLoading = false
        let link = document.createElement('a');
        let body = document.querySelector('body');
        link.href = window.URL.createObjectURL(res);
        link.download = "普通审批菜单记录（第" + args.pageNum + "页）.xlsx";
        // fix Firefox
        link.style.display = 'none';
        body.appendChild(link);
        link.click();
    ...
    importExcel(file){
      ...
      reader.onload = (e) => {
        ...
        approvalEntityApi.importExcel(excelFile).then(res=>{
          if (res.code === '000000') {
            this.$message({
              type: 'success',
              message: '导入成功!'
