  ...
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-form-item label="名称">
          <el-input v-model="listQuery.treeName" placeholder="名称"
                    class="filter-item"
                    @keyup.enter.native="handleQuery"/>
      </el-form-item>
...
<div class="table-container">
    <el-table  :data="list" highlight-current-row
      ...
      <el-table-column label="操作" align="center" width="150" fixed="right">
        ...
        <!-- 查看表单 -->
        <treeEntity1-show ref="treeEntity1Show"/>
        <!-- 新建表单 -->
        <treeEntity1-add ref="treeEntity1Add"
                                :treeList = "treeOptions"
                                @created="doQueryList({})"/>
        <!-- 编辑表单 -->
        <treeEntity1-edit ref="treeEntity1Edit"
                                :treeList = "treeOptions"
                                @updated="doQueryList({})"/>
    <!-- 详情表单 -->
    <Tabs ref="tabs" :params="params" @doQueryList="doQueryList({})"/>
    <!-- 撤销表单 -->
    <treeEntity1-undo ref="treeEntity1Undo" @undo="doQueryList({})"/>
  </div>
</template>

<script>
import treeEntity1Add from './add'
import treeEntity1Edit from './edit'
import treeEntity1Show from './show'
import Tabs from '@/components/Tabs/index'
import treeEntity1Undo from './undo'
...
export default {
  ...
  data() {
    return {
      params: {
        'businessKey': '7c426a9a28004710a0ef1dcf75d1f9af',
  ...
  methods: {
    ...
    doQueryList() {
      ...
      this.listLoading = true
      return treeEntity1Api.treeList(args)
        .then(res => {
          this.list = res.data
    ...
    handleDeleteBatch() {
      if (this.selectItems.length <= 0) {
        this.$common.showMsg('warning', '请选择树形工作流菜单')
        return
      }
      return this.$common.confirm('是否确认删除')
        .then(() =>{
            this.listLoading = true
            treeEntity1Api.deleteBatch(this.selectItems.map(row => row.id))
            .then(() => {
                this.$common.showMsg('success', '删除成功')
                return this.doQueryList()
            }).catch(()=>{
     ...
     */
    handleCreate() {
      this.$refs.treeEntity1Add.handleCreate()
    },
    /**
     * 打开子菜单
     */
    handleCreateChild(row) {
      this.$refs.treeEntity1Add.handleCreateChild(row.id)
    },
    /**
     * 打开查看表单
     */
    handleShow(row) {
      this.$refs.treeEntity1Show.handleShow(row)
    },
    /**
     * 打开编辑表单
     */
    handleUpdate(row) {
      this.$refs.treeEntity1Edit.handleUpdate(row)
    },
    ...
    handleSubmit(row) {
      this.$common.confirm('是否确认提交')
        .then(()=>{
          ...
          this.remoteCallPayload.url = 'http://' + defaultSettings.gateway + '/platofrmTest11/treeEntity1/updateByWorkflow'
          this.remoteCallPayload.token = 'Bearer ' + getAccessToken()

          form.remoteCallPayload = this.remoteCallPayload
          delete form.hiddenAttrs
          //调用提交接口
          treeEntity1Api.submit(form).then(res=>{
            //未找到流程实例
            if('000000' === res.code){
                this.doQueryList({ page: 1 });
                this.$message({
    ...
    handleDetails(row) {
      ...
      const params = {
        'businessKey': '7c426a9a28004710a0ef1dcf75d1f9af' + row.id,
        'variable-name': 'workflowPrefix_form'
      }
     ...
     */
    handleUndo(row) {
      this.$refs.treeEntity1Undo.handleCreate(this.params.businessKey + row.id)
    },
    /**
     * 结束业务
     *
    ...
    handEndProcess(row) {
      this.$common.confirm('未找到流程实例,是否结束业务')
        .then(()=>{
          this.listLoading = true
          treeEntity1Api.endProcess(row).then(res=>{
            if(res.code === '000000'){
              this.doQueryList({ page: 1 });
              this.$message({
                type: 'success',
    ...
    handleRushDo(row) {
      this.$common.confirm('是否催办')
        .then(()=>{
          this.listLoading = true
          treeEntity1Api.rushDo(this.params.businessKey + row.id).then(res=>{
            if(res.code === '000000'){
              this.$message({
                type: 'success',
                message: '催办成功!'
