<template>
  <div class="svg-Icons-container">
    ...
    />
    <div v-if="icon" class="element-icons-list">

      <div v-if="!searchMode" class="d2-icon-select--group">
        <el-row class="d2-icon-select--class-row">
          <el-col v-for="(iconName, iconIndex) in icon" :key="iconIndex" class="d2-icon-select--class-col" :span="6" @click.native="selectIcon(iconName)">
      ...
      <div v-if="searchMode" class="d2-icon-select--group">
        <el-row class="d2-icon-select--class-row">
          <el-col v-for="(iconName, iconIndex) in iconFilted" :key="iconIndex" class="d2-icon-select--class-col" :span="6" @click.native="selectIcon(iconName)">
            <svg-icon :icon-class="''+iconName" class-name="disabled" />
            <span>{{ iconName }}</span>
          </el-col>
        </el-row>
...
import icon from './icons/svg-icons'
export default {
  name: 'SvgIcons',
